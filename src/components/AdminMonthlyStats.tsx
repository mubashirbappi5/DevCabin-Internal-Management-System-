...
const AdminMonthlyStats = ({ refreshKey = 0 }: AdminMonthlyStatsProps = {}) => {
  ...
  // ✅ FIX: Function moved above useEffect to avoid ReferenceError
    ...
    try {
      ...
      const stats: MemberStats[] = members?.map(member => {
        ...
        const attendanceRate = totalWorkingDays > 0
          ? ((presentDays + leaveDaysCount) / totalWorkingDays) * 100
          : 0;
  ...
  if (!profile || !['admin', 'project_manager'].includes(profile.role)) return null;
  ...
  const teamAverages = {
    ...
    averageAttendance: memberStats.length > 0
      ? Math.round((memberStats.reduce((sum, m) => sum + m.attendanceRate, 0) / memberStats.length) * 10) / 10
      : 0,
  ...
  return (
    <Card>
      <CardHeader>
        ...
        <CardDescription>Attendance statistics for all team members</CardDescription>
      </CardHeader>
      <CardContent className="space-y-6">
        ...
        <div className="flex items-center justify-between p-4 bg-primary/10 rounded-lg">
          ...
          <div className="flex items-center gap-2">
            ...
            <Badge variant={getAttendanceRateBadge(teamAverages.averageAttendance)}>
